...
const RecordAction: React.FC = () => {
    ...
    const action =
        data?.record &&
        data.record.recordActions.find((r) => r.name === actionName)

    const fetchRecord = (): void => {
        setLoading(true)
        api.recordAction(match.params)
            .then((response) => {
                setLoading(false)
    ...
    // wrong data.
    const hasDifferentRecord =
        data?.record && data.record.id.toString() !== recordId
